...
class otQuery():
    ...
    def initQuery(self):
        self.headers = {'Content-Type': 'text/xml', 'charset': 'iso-8859-1',
                        'SOAPAction': '"http://www.omninet.de/OtWebSvc/v1/%s"' % (self.command)}
        self.xml = r'<?xml version="1.0" encoding="utf-8"?><soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ' + \
                   ...
                   r'%s</%s></soap:Body></soap:Envelope>' % (self.body, self.command)

    def sendQuery(self):
        result = requests.post(url,
                               data=self.xml.decode(Encoding).encode("ascii", "xmlcharrefreplace").replace(r'\r\n',
                                                                                                           '&#x000d;&#x000a;'),
        ...
        if root.attrib['success'] == "true":
            ...
            #print " %s, %s" % (property,value)
                if isinstance(value, ot_field):
                    ...
                    if len(properties) > 0:
                        value.value = self.convertAttributeforPython(value,value.getValueFromXML(properties[0]))
    ...
    def get(self,item, id):
        ...
        self.body = r'%s<Get folderPath="%s" recursive="true"><ObjectIDs objectIDs="%s"/>'%(self.body,item.folder, id)
        for property, value in vars(self).iteritems():
            #print value
            if isinstance(value,ot_field) and value.name != "objectId":
                self.body = r'%s<RequiredField>%s</RequiredField>' % (self.body, value.name)
    ...
    def convertAttributeforPython(self, field, value):
    ...
    def convertAttributeforOT(self,field):
        if isinstance(field,DateTimeVal):
    ...
    def getItem(self, item, field):
        self.body = ""
        self.command = "GetObjectList"
        self.body = r'%s<Get folderPath="%s" recursive="true"><ObjectIDs objectIDs="%s"/>' % (self.body, self.folder, item.id.value)
        self.body = r'%s<RequiredField>%s</RequiredField>' % (self.body, field.name)
        self.body = "%s</Get>" % (self.body)
        self.initQuery()
        self.sendQuery()
    ...
    def update(self,item,field):
        value = self.convertAttributeforOT(field)
        if item.id == "":
            return False
        self.body = ""
        self.command = "ModifyObject"
        self.body = r'%s<Object objectId="%s">' % (self.body, item.id)
        self.body = r'%s<%s name="%s">%s</%s>' % (self.body, field.fieldtype, field.name, value, field.fieldtype)
        self.body = "%s</Object>" % (self.body)
        self.initQuery()
        self.sendQuery()
        print self.xml
        ...
        #print "looking for property %s" % item
        if root.attrib['success'] == "true":
            self.result = True
        else:
    ...
    def delete(self,id):
        ...
        self.body = r'%s<ObjectID>%s</ObjectID><IgnoreReferences>true</IgnoreReferences>' % (self.body, id)
        self.initQuery()
        self.sendQuery()
        tree = ET.fromstring(self.xml_result)
