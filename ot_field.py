...
class ot_field(object):
    ...
    def fieldXMLString(self,fieldname):
        fieldquery = r'<%s name="%s">%s</%s>' % (self.fieldtype, fieldname, self.value, self.fieldtype)
        return fieldquery
...
class ReferenceVal(ot_field):
    ...
    def fieldXMLString(self, fieldname):
        fieldquery = r'<ReferenceVal name="%s" objectId="%s"/>' % (self.name, self.value)
        return fieldquery
    def getValueFromXML(self,xml):
...
class ReferenceToUserVal(ot_field):
    ...
    def fieldXMLString(self,fieldname):
        fieldquery = r'<ReferenceToUserVal name = "%s" type = "%s" Value = "%s" />' % (self.fieldtype, self.reference, self.value)
        return fieldquery
    def getValueFromXML(self, xml):
